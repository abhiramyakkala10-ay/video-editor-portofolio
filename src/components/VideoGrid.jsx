...
const VideoGrid = ({ videos }) => {
  ...
  // Helper to check if URL is a Gumlet embed
  const isEmbedUrl = (url) => {
    return url.includes('gumlet.com/embed') || url.includes('youtube.com/embed') || url.includes('player.vimeo.com');
  };
    ...
    <div id="work" className="container" style={{ paddingBottom: '100px' }}>
      ...
      {videos.length === 0 ? (
        <div style={{ textAlign: 'center', color: 'var(--text-secondary)', padding: '40px' }}>
          No videos published yet. Double-click the logo to access Studio and add your first work!
        </div>
      ) : (
      ...
      <AnimatePresence>
        {activeVideo && (
          ...
          >
            <button 
              style={{ position: 'absolute', top: '24px', right: '24px', background: 'rgba(255,255,255,0.1)', padding: '12px', border: 'none', borderRadius: '50%', cursor: 'pointer', zIndex: 1001 }}
              onClick={() => setActiveVideo(null)}
            >
            ...
            >
              {isEmbedUrl(activeVideo.video_url) ? (
                <iframe 
